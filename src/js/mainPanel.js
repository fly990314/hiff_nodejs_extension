...
const get_current_html_frommain_to_content_btn = document.getElementById("get_current_html_frommain_to_content");
...
get_current_html_frommain_to_content_btn.addEventListener('click', 
() => {
    console.log("click get_current_html_frommain_to_content");
    chrome.tabs.query({ active: true }, tabs => {
        ...
        chrome.tabs.sendMessage(
            tabID,
            {
            type: 'Get_CurrentHTML_In_MainPanel',
            payload:{ message: 'html file request in background.' }
            },
            response => { }
        );
        });
...
chrome.runtime.onMessage.addListener( 
    ...
    {
        if (sender_package.type === 'Finish_Content_In_Background') {
            console.log( sender_package.payload.message );

            return_sender_response( { msg: "Finish MainPanel." } );
            }
    }
);