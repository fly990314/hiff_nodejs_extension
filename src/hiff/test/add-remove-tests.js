...
function assertOneAddition(diff) {
  ...
  assert.equal(diff.added_type[0].type, 'added');
}
...
function assertOneRemoval(diff) {
  ...
  assert.equal(diff.removed_type[0].type, 'removed');
}
...
describe("Multiple additions/removals", function() {
  it("should be correctly recognized", function() {
    ...
    assert.equal(diff.added_type.length, 4);
    assert.equal(diff.removed_type.length, 1);
    assert.equal(diff.changed_type.length, 0);

    var addedTypes = _.map(diff.added_type, 'type');
    assert.deepEqual(addedTypes, ['added', 'added', 'added', 'added']);

    var removedTypes = _.map(diff.removed_type, 'type');
    assert.deepEqual(removedTypes, ['removed']);
